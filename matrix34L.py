...
# New dictionary to map simple command names to their full launch commands
# Added a more robust entry for VSC
LAUNCH_COMMANDS = {
    "vsc": "flatpak run com.visualstudio.code",
    "vivaldi": "flatpak run com.vivaldi.Vivaldi",
    "notepad.py": "python3 notepad.py",
    "tetris.py": "python3 tetris.py",
    "spotify": "flatpak run com.spotify.Client", # Or use the command that worked for you
    "thonny": "thonny", # Or the path found by the 'which' command
    "rstudio": "rstudio"
}
...
def main(stdscr):
    ...
    while True:
        if fade_out_active and time.time() >= fade_out_timer:
            ...
            # End curses mode to allow the new program to run
            curses.endwin()
            
            try:
                # Use the LAUNCH_COMMANDS dictionary to get the correct launch command
                command = LAUNCH_COMMANDS.get(APP_TO_LAUNCH)
                
                if command:
                    # Pass the command as a string to the shell
                    subprocess.run(command, shell=True)
                else:
                    # Fallback for dynamic open command, assuming it's a simple executable name
                    subprocess.run(APP_TO_LAUNCH, shell=True)
                
                OUTPUT_MESSAGE = f"{APP_TO_LAUNCH} has ended."
            except FileNotFoundError:
                OUTPUT_MESSAGE = f"Error: Program '{APP_TO_LAUNCH}' not found."
